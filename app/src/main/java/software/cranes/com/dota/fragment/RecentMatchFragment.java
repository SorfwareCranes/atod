 ...
 */
public class RecentMatchFragment extends BaseFragment {
    ...
    private void findViews(View view) {
        ...
        query = mFirebaseDatabase.getReference("pro/match").orderByChild("time").limitToLast(100);
        FirebaseRecyclerAdapter<MatchModel, ViewHolderMatchModel> adapter = new FirebaseRecyclerAdapter<MatchModel, ViewHolderMatchModel>(MatchModel.class, R.layout.match_row_layout, ViewHolderMatchModel.class, query) {
            ...
            protected void populateViewHolder(ViewHolderMatchModel viewHolder, final MatchModel model, int position) {
                ...
                if (model.getTa().getPt() != null && !model.getTa().getPt().equals(Constant.NO_IMAGE)) {
                    ...
                    new ImageRequestCustom(getContext(), viewHolder.imgTeamA, url, sizeImage, sizeImage, R.drawable.no_image_team, bitmapRes).execute(viewHolder.imgTeamA);
                }
                if (model.getTb().getPt() != null && !model.getTb().getPt().equals(Constant.NO_IMAGE)) {
                    ...
                    new ImageRequestCustom(getContext(), viewHolder.imgTeamB, url, sizeImage, sizeImage, R.drawable.no_image_team, bitmapRes).execute(viewHolder.imgTeamB);
                }
                if (model.getSum() > 0) {
                    viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            BaseFragment fragment = new DetailEndMatchFragment();
